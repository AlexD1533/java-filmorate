...
public class LikeRepository extends BaseRepository<Like> {
    ...
    private static final String DELETE_BY_USER_ID_SQL = "DELETE FROM likes WHERE user_id = ?";
    private static final String COUNT_BY_FILM_ID_SQL = "SELECT COUNT(*) FROM likes WHERE film_id = ?";
    ...
    public Set<Long> findFilmIdsByUserId(Long userId) {
        List<Long> filmIds = jdbcTemplate.queryForList(FIND_FILM_IDS_BY_USER_ID_SQL, Long.class, userId);
    ...
    public void deleteByFilmId(Long filmId) {
        jdbcTemplate.update(DELETE_BY_FILM_ID_SQL, filmId);
    }
    ...
    public int countLikesByFilmId(Long filmId) {
        Integer count = jdbcTemplate.queryForObject(COUNT_BY_FILM_ID_SQL, Integer.class, filmId);
